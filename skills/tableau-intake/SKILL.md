...
1. **Precheck.** From the project directory, run:
   ...
   ```bash
   python "${CLAUDE_PLUGIN_ROOT}/skills/tableau-intake/scripts/intake.py" precheck "<project-dir>"
   ```
...
5. **Commit** — only after the analyst approves (or chooses to skip):
   ...
   ```bash
   python "${CLAUDE_PLUGIN_ROOT}/skills/tableau-intake/scripts/intake.py" commit "<project-dir>" --status approved
   # or, to skip the step:
   python "${CLAUDE_PLUGIN_ROOT}/skills/tableau-intake/scripts/intake.py" commit "<project-dir>" --status skipped
   ```
